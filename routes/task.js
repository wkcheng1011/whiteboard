...
router.get(/\/attempt\/(.{36})/, async (req, res) => {
	if (!req.session.user) {
		...
		const meta = await db.get(
			"select users.name as user_name, tasks.name as task_name, attempts.id as attempt_id, classes.name as class_name, * from attempts, tasks, users, classes where attempts.id = ? and attempts.task_id = tasks.id and attempts.user_id = users.id and tasks.class_id = classes.id",
			id
		);

		const questions = shuffle(
			await db.all(
				"select * from questions where task_id = (select task_id from attempts where id = ?)",
				id
			)
		);
		for (const question of questions) {
			...
			const selectedAnswer = await db.get(
				"select attemptAnswers.answer_id from attempts, attemptAnswers where attempts.id = ? and attemptAnswers.attempt_id = attempts.id and attemptAnswers.answer_id in (select id from answers where answers.question_id = ?)",
				id,
				question.id
			);
...
router.get(/\/contents\/(.{36})/, async (req, res) => {
	if (!req.session.user) {
		...
		if (req.session.user.type == 0) {
			...
			const questions = shuffle(
				await db.all("select * from questions where task_id = ?", id)
			);
			for (const question of questions) {
				const answers = await db.all(
					"select * from answers where question_id = ?",
...
router.post(/\/contents\/(.{36})/, async (req, res) => {
	if (!req.session.user) {
		...
		const _uuid = uuid();
		await db.run(
			"insert into attempts (id, task_id, user_id) values (?, ?, ?)",
			_uuid,
			id,
			req.session.user.id
		);
		...
		for (const question_id in req.body) {
			...
			await db.run(
				"insert into attemptAnswers values (?, ?)",
				_uuid,
				answer_id
			);
		}
...
router.get(/\/summary\/(.{36})/, async (req, res) => {
	if (!req.session.user) {
		...
		const attempts = await db.all(
			"select sum(correct) as s, count(*) as c, attempts.id as attempt_id, users.id as user_id, * from attempts, users, attemptAnswers, answers where task_id = ? and user_id = users.id and attempts.id = attemptAnswers.attempt_id and attemptAnswers.answer_id = answers.id group by attempts.user_id",
			id
		);
...
router.get("/", async (req, res) => {
	if (!req.session.user) {
		...
		for (const task of tasks) {
			...
			task.average =
				attemptAnswers.filter((a) => a.correct).length / attemptAnswers.length;
		}
...
router.post("/new", async (req, res) => {
	if (!req.session.user) {
		...
		for (const question_id in questions) {
			...
			for (const answer_id of Object.keys(question).filter((a) =>
				a.startsWith("a-")
			)) {

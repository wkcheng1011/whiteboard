...
router.get("/", async (req, res) => {
	if (!req.session.user) {
		...
		const sentMessages = await db.all(
			"select * from messages where from_id = ? and type = 0",
			req.session.user.id
		);
		const receivedMessages = await db.all(
			"select * from messages where to_id = ? and type = 0",
			req.session.user.id
		);

		for (const message of sentMessages) {
			counts[message.to_id] = counts[message.to_id] || message;
			counts[message.to_id].name = users.filter(
				(_user) => _user.id == message.to_id
			)[0].name;
		...
		for (const message of receivedMessages) {
			...
			counts[message.from_id].name = users.filter(
				(_user) => _user.id == message.from_id
			)[0].name;
		...
		const classMessages = (
			await db.all(
				"select count(*) as cnt, * from messages where type = 1 group by to_id order by at desc"
			)
		).map((message) => {
			message.name = classes.filter(
				(_class) => _class.id == message.to_id
			)[0].name;
		...
		return res.render("message", {
			messages: [...Object.values(counts), ...classMessages].sort((m1, m2) => {
...
router.get(/\/(\d)\/(.{36})/, async (req, res) => {
	if (!req.session.user) {
		...
		if (type == 0) {
			// DM
			displayname = (await db.get("select name from users where id = ?", id))
				.name;
			messages = await db.all(
				"select * from messages, users where (from_id = ? and to_id = ? or from_id = ? and to_id = ?) and from_id = users.id",
				id,
				req.session.user.id,
				req.session.user.id,
				id
			...
			// GM
			displayname = (await db.get("select name from classes where id = ?", id))
				.name;
			messages = await db.all(
				"select classes.name as displayname, users.name as user_name, * from messages, classes, users where to_id = ? and to_id = classes.id and from_id = users.id",
				id
			);
		}
